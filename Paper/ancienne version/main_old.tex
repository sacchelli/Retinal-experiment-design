...
\newcommand{\Mohamad}[1]{{\textcolor{magenta}{[Mohamad: \textit{#1}]}}}
...
\title{Optimal experiment design for parameter estimation in linear noisy networks}
\author{
B. Cessac, L. Pronzato, L. Sacchelli, M. Samman
}
\date{\today}
...
\begin{abstract}
    %We do stuff.
    \B{Il faudrait commencer a preciser:
    %
    \begin{itemize}
    \item Quels sont les motivations ? 
    \item A qui on s'adresse ? \LS{il semble que nous avons choisi le lectorat de Automatica en premier lieu}
    \item Quel est le resultat cle du papier ?
    \end{itemize}
    }
\end{abstract}
...
This article is motivated by the following problem. Contemporary technology in neuroscience allows for experiments known as “closed-loop” experiments. One application concerns the characterisation of the retina. This membrane, which lines the back of the eye and is approximately half a millimeter thick (in humans), is capable of converting photons from the outside world into trains of impulses (action potentials) that are transmitted to the brain via the optic nerve. To better understand how the structure and dynamics of this organ enable it to optimally encode visual scenes, it is subjected to different types of stimuli and the response is recorded, so that mathematical models can then be developed. In closed loop experiments, the retina is subjected to simple stimuli (such as “white noise”), and the response is recorded, which is then used to adjust the parameters of a preliminary model. Based on this model, a new stimulus is generated using an optimality criterion. This new stimulus is then subjected to the retina, and so on. \B{Ajouter refs.}

However, this approach encounters several problems. 
\begin{enumerate}
    \item Firstly, the experiments (conducted on animals) are performed \textit{ex vivo}. Thus, even if the retina is immersed in a preservative medium, it only remains functional for a few hours. This means that measurements, parameter estimates/model adjustments, and stimulus determinations must be performed over short periods of time, with the aim of obtaining at least ten loops per experiment. 
    \item The models have a large dimensionality. In fact, closed-loop experiments typically aim to determine the properties of individual neurons in the retina. But more recently, and this is the case here, researchers wanted to determine the collective properties of a large number of cells (several hundred) interacting in a network. This implies high dimensionality in phase space. The parameter space can initially be reduced by using simplified phenomenological models \cite{kartsaki-hilgen-etal:24}, as we use here.
    \item Optimality criteria are usually defined via the characteristics of individual cells, for example, maximum activity achieved for specific "preferred" stimuli. We would like to develop optimality criteria that take into account the \textit{collective dynamics} of retinal cells interacting via a synaptic network in response to non-stationary stimuli. Furthermore, this criterion should incorporate a notion of optimality in the way visual \textit{information} is encoded. 
\end{enumerate}

In this article, we address these questions from the natural perspective of optimal experiment design theory, within the framework of a retinal model proposed in \cite{kartsaki-hilgen-etal:24}. For simplicity, we consider here a linear version of this model, subjected to stimuli that are generally non-homogeneous and spatially non-stationary, under the influence of Brownian noise. We formulate the problem of parameter estimation and stimulus optimality in this context. The Fisher information matrix plays a central role in this approach, and we show that, in this model, it can be calculated explicitly, along with its first and second derivatives, using a numerically efficient procedure. It naturally arises in the parameter estimation problem, via a central limit theorem, but also in the determination of optimal stimuli. Indeed, potential optimality criteria are defined as functions of this matrix \cite{}. We study the mathematical properties of a criterion based on the determinant of this matrix, properties that are particularly relevant to the numerical problem of stimulus design. We also discuss how such optimality criteria, explicitly involving spatio-temporal correlations induced by collective dynamics (called “noise correlations” in the neuroscience literature), may offer better options than those currently used in the study of the retina by closed-loop stimulation.
...
\label{S:discretization}

Let $n,m,p,q\in \N$. Let $\Theta\subset \R^p$ be the set of admissible parameters. We set $A:\Theta\to \R^{n\times n}$, $B\in \R^{n\times m}$, $C\in \R^{q\times n}$, and $D\in \R^{n\times n}$. We assume the mapping $A$ to be of regularity $C^1$ at least. Let $T>0$ and let $u\in L^\infty([0,T],\R^m)$ (for the presumed Lebesgue measure). Let $W$ be a standard $n$-dimensional Brownian motion. For a given parameter $\theta\in \Theta$,  we consider the controlled stochastic differential equation:
\begin{equation}\label{E:sde1}
\diff x(t)=A(\theta) x(t) + B u(t)+D \diff W(t).
\end{equation}
Given a deterministic initial condition $x(0)=x_0\in \R^n$, \eqref{E:sde1} admits a unique strong solution  (see, e.g., \cite{oksendal2013stochastic}), given by, for any $t\in [0,T]$,
\begin{equation}\label{E:VarConst}
...
There exists a $\Delta$-periodic, essentially-bounded function $U:\R\to \R^{m\times m}$, and a sequence $(u_k)_{0\leq k<N}\in \left(\R^m\right)^N$ such that: 
\begin{equation}
    u(t)=U(t)u_k, \quad  t\in [t_k,t_{k+1}), 0\leq k< N.
\end{equation}
We denote by $\U$ the set of these functions (the inputs). \LP{ N'est-ce pas plutôt l'ensemble des $u_k$?}
\end{assumption}
Set
$$
F=e^{\Delta A(\theta)},
...
$$
\Sigma = \int_0^\Delta e^{A(\theta)(\Delta-s)}D D^\top  e^{A^\top(\theta)(\Delta-s)}\diff s.
$$
...
$$
$$
\dot{\tilde{\Sigma}}(t)= A(\theta)\tilde{\Sigma}(t)+\tilde{\Sigma}(t)A(\theta)^{\top}+D D^\top ,\quad {\tilde{\Sigma}}(0)=0\in \R^{n\times n}.
$$
\end{remark}
...
y_k(u)
=\eta(t_k,\theta,u)+\sum_{l=0}^{k-1} C F^{k-l-1}W_{l}+\varepsilon_k,
\end{equation}
with $\eta$ denoting the deterministic output
\begin{equation}\label{E:output}
\eta(t_k,\theta,u)
=
CF^{k}x_0+\sum_{l=0}^{k-1} C F^{k-l-1}Gu_{l} .
\end{equation}
...
\Sigmab
=
\mathrm{Diag}(C,\dots, C)
\cdot\Sb\cdot
\mathrm{Diag}(C,\dots, C)^\top
+
\mathrm{Diag}(\Sigma', \dots, \Sigma'),
$$
...
\bar B u,
$$
%From this observation, following steps similar to that of section~\ref{S:discretization}, the construction of the derivatives of each element with respect to the parameters follows.
letting 
...
=
\mathrm{Diag}(\bar C,\dots, \bar C)
\cdot \Sb \cdot
\mathrm{Diag}(\bar C',\dots, \bar C')^\top
+
\mathrm{Diag}(\bar C',\dots, \bar C')
\cdot \Sb \cdot
\mathrm{Diag}(\bar C,\dots, \bar C)^\top.
...
=
    ...
    \end{pmatrix}
    \mathrm{Diag}(\bar G_i,\dots, \bar G_i) \in \R^{Nq\times N m},
\end{equation}
and
\begin{equation}
    K_i
...
\begin{proof}[Proof of Lemma~\ref{L:conv}]
...
= \;&
\mathrm{Diag}(\tilde C,\dots, \tilde C)
\cdot\Sb^{ij}\cdot
\mathrm{Diag}(\tilde C',\dots, \tilde C')^\top
\\&+
\mathrm{Diag}(\tilde C'_i,\dots, \tilde C'_i)
\cdot\Sb^{ij}\cdot
\mathrm{Diag}(\tilde C'_j,\dots, \tilde C'_j)^\top
\\
&+
\mathrm{Diag}(\tilde C'_j,\dots, \tilde C'_j)
\cdot\Sb^{ij}\cdot
\mathrm{Diag}(\tilde C'_i,\dots, \tilde C'_i)^\top
\\&+
\mathrm{Diag}(\tilde C',\dots, \tilde C')
\cdot\Sb^{ij}\cdot
\mathrm{Diag}(\tilde C,\dots, \tilde C)^\top
