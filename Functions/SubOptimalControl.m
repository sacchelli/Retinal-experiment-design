function [outputs,weights] = SubOptimalControl(Acol, B, C, theta, sigma, Sigmap, Delta, controls)
...
minScoreVertex = Inf;
minScorevertexIndex = 0;

for k = vertices
    
    scorek = logdet(M(:,:,k));

    if scorek < minScoreVertex 
        minScoreVertex = scorek;
        minScorevertexIndex = k;
    end
...
outputs = controls(:,:, minScorevertexIndex);
weights = 1;
