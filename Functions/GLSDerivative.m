function [J, gradJ] = GLSDerivative(Acol, B, C, theta, sigma, Sigmap, Delta, controls, weights, y)
...
for k = 1:nU
    ...
    SigmaBoldInvTimesError = SigmaBold\(ybold-etabold);

    Jaux(k) = (ybold-etabold)'*(SigmaBoldInvTimesError)+logdet(SigmaBold);
    ...
    for i = 1:p
        ...
        dJ(i,k) = - 2*(SigmaBoldInvTimesError'*etaboldbi) + (SigmaBoldInvTimesError)'*Sigmaboldbi*(SigmaBoldInvTimesError)+trace(SigmaBold\Sigmaboldbi);
