...
depth = 10; % Seems to be a sweet spot under current values, 
...
nbrRandomInputs = 1000; % must be >0 if true.
...
% positions = [115, 60, 187];

q = 4; % number of outputs


pos4 = floor(sdn/3);
positions = [(pos4-1)*sdn+pos4,(pos4)*sdn-pos4+1,(sdn-pos4)*sdn+pos4,(sdn-pos4+1)*sdn-pos4+1];
...
% 'laplacian2DGraph' connection (unrealistic for the retina): Bij -(-4)-> Aij
...
Cprim = sparse([zeros(dn,2*dn),eye(dn,dn)]);

%C = sparse([zeros(dn,2*dn),eye(dn,dn)]);
%C = sparse([zeros(dn,2*dn),gaussianPooling2DGraph(sdn,delta,5*sigmaAG)]);

%positions = [44, 56, 67, 120, 125, 132, 190, 201, 213];
...
cMax = 30*cStar;   % max wave velocity
cMin = 0; % min wave velocity

kMax = pi/retinalWidth*sdn/2;   % max wave number
...
if addRandomInputs % produce random inputs if asked
    [controlsRandom, inputParametersRandom] = generate2DRandomBinaryControls(sdn, N, nbrRandomInputs, seed);
end
...
maxStepSearch = 2000; % Max number of loops
i = 0;
...
if addRandomInputs
    ...
    Mrand = zeros(p,p);
    for k = K-1:-1:K-1-nbrRandomInputs
...
for k=1:length(idx)
    ...
    info{k,4} = num2str(inputParameters(3,idx(k))/(kStar));
end
...
if createAnimatedGIF
    ...
    gifname   = ['videoOutput/Optimal_inputs_2D_',s,'.gif'];
    delayTime = Delta/1000;
    firstFrame = true;
    ...
    fig = figure(2);
    clf(fig,'reset')
    set(fig,'Position',[20 50 nCols*600 nRows*400])   
    set(gca,'YDir','normal')
