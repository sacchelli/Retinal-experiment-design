clear all;
addpath './Functions'
clc


%rng(10)
%rng(11)

%%%% Data

n = 2;
m = 1;
p = 2;
q = 1;

N = 100;

Tf=2*pi;

prec = 500;   % must be even

Delta = Tf/N;

A0 = [-1, 1; 0, 1];
A1 = eye(n);
A2 = [0, -1; 1, 0];

Acol(:,:,1)=A1;
Acol(:,:,2)=A2;
Acol(:,:,3)=A0;

% sigma = 0.5*eye(n);
% Sigmap = 0.05;

sigma = 0.1*eye(n);
Sigmap = 0.01;

% sigma = 0.01*eye(n);
% Sigmap = 10^(-4);


B = [1; 0];
C = [0, 1];

% theta1 = -1/10;
theta1 = -1;
theta2 = 2;

thetaTrue = [theta1,theta2];


X=[1:N]/N*Tf;
v0 = ones(1,N);
v1 = cos(X/Tf*2*pi);
v2 = sin(X/Tf*2*pi);
v3 = cos(2*X/Tf*2*pi);
v4 = sin(2*X/Tf*2*pi);


density = 2; 

controls = zeros(m,N,2);

counter = 1;
for i0 = -density:1:density
    for i1 = -density:1:density
        for i2 = -density:1:density
            for i3 = -density:1:density
                for i4 = -density:1:density
                            u = (i0*v0+i1*v1+i2*v2+i3*v3+i4*v4);

                            L2u = max(u*u'/N,0.000001);
                            u = u/sqrt(L2u);
                            controls(:,:,counter) = u;
                            counter = counter+1;
                end
            end
        end
    end
end

thetaInit = thetaTrue + 2 * abs(thetaTrue).*((rand(1,2))-0.5);

thetaOld = thetaInit;

%thetaOld = [ -415.0962 -533.3553]/1000;

thetaNew = thetaOld;

step=0.01;

estimate = [];

estimateNaive = [];

%%

for k=1:50

    
    [inputs,weights] = OptimalControls(Acol, B, C, thetaOld, sigma, Sigmap, Delta, controls);
    

    y = OutputGeneration(Acol, B, C, thetaTrue, sigma, Sigmap, Delta, inputs);
    
    
    %%%% Differentiating the output

    A=Acol(:,:,p+1);
    for i = 1:p
        A=A+thetaOld(i)*Acol(:,:,i);
    end
    
    
    F = buildF(A, Delta);
    G = buildG(A, B, Delta, prec);
    Sigma = buildSigma(A, sigma, Delta, prec);
    SigmaBold = buildSigmaBold(C/2, C, F, Sigma, Sigmap, N);
        
    
    [Jf, gradJf] = GLSDerivative(Acol, B, C, thetaOld, sigma, Sigmap, Delta, inputs, weights, y);

    
    %thetaNew = thetaOld + step * gradJf';
    normalizedGradJf = gradJf/(10+norm(gradJf));
    thetaNew = thetaOld - step * normalizedGradJf';
    thetaOld = thetaNew
    display(k/50)
    estimate=[estimate;thetaOld];
end


thetaOld = thetaInit;

%thetaOld = [ -415.0962 -533.3553]/1000;

thetaNew = thetaOld;

step=0.01;


for k=1:50
    
    inputs = controls(:,:,[fix(size(controls,3)/4),2*fix(size(controls,3)/4),3*fix(size(controls,3)/4)]);

    weights = [1/2,1/4,1/4];

    y = OutputGeneration(Acol, B, C, thetaTrue, sigma, Sigmap, Delta, inputs);
    
    
    %%%% Differentiating the output

    A=Acol(:,:,p+1);
    for i = 1:p
        A=A+thetaOld(i)*Acol(:,:,i);
    end
    
    
    F = buildF(A, Delta);
    G = buildG(A, B, Delta, prec);
    Sigma = buildSigma(A, sigma, Delta, prec);
    SigmaBold = buildSigmaBold(C/2, C, F, Sigma, Sigmap, N);
        
    
    [Jf, gradJf] = GLSDerivative(Acol, B, C, thetaOld, sigma, Sigmap, Delta, inputs, weights, y);

    
    %thetaNew = thetaOld + step * gradJf';
    normalizedGradJf = gradJf/(10+norm(gradJf));
    thetaNew = thetaOld - step * normalizedGradJf';
    thetaOld = thetaNew
    display(k/50)
    estimateNaive=[estimateNaive;thetaOld];
end


%%

figure(1)
clf
plot(estimate(:,1),estimate(:,2),'b')
hold on
plot(estimateNaive(:,1),estimateNaive(:,2),'r')

figure(2)
clf
plot(sqrt((estimate(:,1)-thetaTrue(1)).^2+(estimate(:,2)-thetaTrue(2)).^2),'b')
hold on
plot(sqrt((estimateNaive(:,1)-thetaTrue(1)).^2+(estimateNaive(:,2)-thetaTrue(2)).^2),'r')






%%%% Plotting

% 
% % T1min = theta1-30*abs(theta1);
% % T1max = theta1+30*abs(theta1);
% % 
% % T2min = theta2-0.4*abs(theta2);
% % T2max = theta2+0.4*abs(theta2);
% 
% T1min = -2;
% T1max = -0.0001;
% 
% T2min = 0;
% T2max = 3;
% 
% nb=20;
% 
% dT1 = (T1max-T1min)/nb;
% dT2 = (T2max-T2min)/nb;
% 
% T1 = T1min : dT1 : T1max;
% T2 = T2min : dT2 : T2max;
% 
% [Theta1,Theta2] = meshgrid(T1,T2);
% 
% Jsurf=zeros(length(T2),length(T1));
% tic
% for i = 1:length(T1)
%     for j = 1:length(T2)
%         Abis = A0+ T1(i)*A1+T2(j)*A2;       
%         %if max(real(eig(Abis)))>0
%         %    Jsurf(j,i)=Inf;
%         %else
%             Jsurf(j,i) = GLSValue(Acol, B, C, [T1(i),T2(j)], sigma, Sigmap, Delta, inputs, weights, y);
%         %end
%     end
% end 
% toc
% 
% 
% 
% 
% 
% inputs2 = controls(:,:,randi(size(controls,3),3,1));
% 
% weights2 = rand(1,3);
% weights2 = weights2/sum(weights2);
% 
% Jsurf2=zeros(length(T2),length(T1));
% tic
% for i = 1:length(T1)
%     for j = 1:length(T2)
%         Abis = A0+ T1(i)*A1+T2(j)*A2;       
%         %if max(real(eig(Abis)))>0
%         %    Jsurf(j,i)=Inf;
%         %else
%             Jsurf2(j,i) = GLSValue(Acol, B, C, [T1(i),T2(j)], sigma, Sigmap, Delta, inputs2, weights2, y);
%         %end
%     end
% end 
% toc
% 
% 
% 
% figure(2)
% clf
% 
% surf(Theta1, Theta2, Jsurf);
% colormap("parula");
% shading("interp");
% 
% %hold on
% 
% figure(3)
% clf
% 
% surf(Theta1, Theta2, Jsurf2);
% colormap( "parula");
% shading("interp");
% 
% 
% % figure(1)
% % 
% % for j = 1:length(T1)
% % 
% % 
% %     theta = [T1(j),theta2];
% % 
% %     A=Acol(:,:,p+1);
% %     for i = 1:p
% %         A=A+theta(i)*Acol(:,:,i);
% %     end
% %     F = buildF(A, Delta);
% %     G = buildG(A, B, Delta, prec);
% %     Sigma = buildSigma(A, sigma, Delta, prec);
% %     SigmaBold = buildSigmaBold(C/2, C, F, Sigma, Sigmap, N);
% %     SigmaBoldinv = inv(SigmaBold);
% % 
% %     for k = 1:p*(p+1)/2
% % 
% %         v = inputs(:,:,k);
% %         ybold = y(:,:,k);
% %         ybold = ybold(:);
% %         % eta(theta)
% % 
% %         eta = zeros(q,N);
% % 
% %         xt = zeros(n,1);
% %         for i=1:N
% %             xt = F*xt + G*v(:,i);
% %             eta(:,i) = C*xt;
% %         end
% %         etabold = eta(:);
% % 
% %         plot(X,[0;etabold],color(k))
% %     end
% %     display(GLSValue(Acol, B, C, theta, sigma, Sigmap, Delta, inputs, weights, y))
% % 
% % end
% 
% 
% %